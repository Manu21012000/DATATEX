...
def collect_data(
    ...
    if not os.path.isfile(data_path):
        ...
        abs_requested = os.path.abspath(data_path)
        msg = (
            f"CSV file not found: {abs_requested}. "
            f"Copy your dataset into WORKING_DIRECTORY ({abs_wd}) and use "
            f"datapath:YourFileName.csv matching that filename."
        )
        logger.error(msg)
    ...
    raise ValueError(
        f"Unable to read CSV with encodings {encodings}. Last error: {last_error}"
    )
