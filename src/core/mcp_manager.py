...
# Serialize MCP connect/call/disconnect across threads. Each MCPToolAdapter.invoke may use
# asyncio.run() in a worker thread; sessions must not be shared across event loops.
MCP_CROSS_THREAD_LOCK = threading.Lock()
...
def _mcp_server_dependencies_met(server_name: str) -> bool:
    """Return False if optional API keys are missing (do not spawn npx)."""
    if server_name == "web-search":
        tav = os.getenv("TAVILY_API_KEY", "").strip()
        if not tav or "XXXX" in tav.upper():
            logger.warning(
                "Skipping MCP server web-search: set TAVILY_API_KEY in .env "
                "(optional; get a key at https://tavily.com)."
            )
            return False
    if server_name == "github":
        tok = os.getenv("GITHUB_TOKEN", "").strip()
        if not tok or "XXXX" in tok.upper():
            logger.warning(
                "Skipping MCP server github: set GITHUB_TOKEN in .env "
...
# npx + first-time package download can exceed 30s on Windows
    ...
    async def connect(self, server_name: str) -> bool:
        ...
        async with self._connection_locks[server_name]:
            ...
            try:
                ...
                # Long-lived connection: manual __aenter__; on any failure we MUST
                # __aexit__ both contexts in this task or stdio/anyio raises cancel-scope errors.
                ...
                try:
                    ...
                    try:
                        if session_entered and session_context is not None:
                            await session_context.__aexit__(*exc_info)
                        if client_entered:
                            await client_context.__aexit__(*exc_info)
                    except Exception as cleanup_err:
                        logger.warning(
                            "Error cleaning up failed MCP connect (%s): %s",
                            server_name,
                            cleanup_err,
                        )
                    if isinstance(exc_info[1], asyncio.TimeoutError):
                        logger.error(
                            "Timeout connecting to MCP server: %s "
                            "(ensure Node/npx is installed and on PATH; first npx run can be slow)",
                            server_name,
                        )
                    else:
                        logger.error(
        ...
        with MCP_CROSS_THREAD_LOCK:
            try:
                ...
                with concurrent.futures.ThreadPoolExecutor(max_workers=1) as pool:
                    return pool.submit(asyncio.run, _gather_tools()).result(timeout=180)
            except Exception as e:
